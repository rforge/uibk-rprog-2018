...
\section{Circular regression models}\label{sec:circmax}
...
parameter estimation on the given data is given in the \pkg{circmax} package so that MOB algorithm can 
