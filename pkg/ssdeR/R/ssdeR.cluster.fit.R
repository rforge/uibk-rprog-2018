...
# ssdeR.cluster.fit
########################
ssdeR.cluster.fit <- function(fm, dfcw, cluster)
{
  if (NCOL(cluster) == 1 ) {
    cluster = cluster[rownames(cluster) %in% names(weights(fm)),1]
    ...
    cluster1 = cluster[rownames(cluster) %in% names(weights(fm)),1]
    cluster2 = cluster[rownames(cluster) %in% names(weights(fm)),2]
