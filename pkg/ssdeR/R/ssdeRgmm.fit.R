                         ...
                         control = control)
{
  ...
  if (is.null(weights))
    weights <- rep.int(1, length(y3[!is.na(y3)]))else{
      weights <- weights
    }
  ...
  y_tilde <- rw * y3    ## weighted response

  theta_2  = solve(t(X_tilde)%*%(X_tilde))%*%t(X_tilde)%*%(y_tilde)
  # check: lm.fit(X_tilde, y_tilde)
  ...
  ll<-0.5 * (sum(log(w)) - n * (log(2 * pi) + 1 - log(n) + log(sum(w * (y3 - fit$mu)^2))))
