                 ...
                 model = TRUE, x = FALSE, y = FALSE, ...)
{
  ...
  # little bit 'dirty' but does the job and offers intuitive formula design
  # tweek later

  formula <- as.formula(paste(paste(formula[2]), paste(formula[3], paste(treatment[[2]]), sep = " + "), sep=" ~ "))
  selection <- as.formula(paste(paste(selection[2]), paste(selection[3], paste(treatment[[2]]), sep = " + "), sep=" ~ "))

  # use Formula Package to get the input selection straight
  ...
  Y3 <- model.part(formula, data = mf, lhs = 1, drop = T)
  Y2 <- model.part(formula, data = mf, lhs = 2, drop = T)
  Y1 <- model.part(formula, data = mf, lhs = 3, drop = T)
  ...
  if( !is.null( weights ) ) {
    if( length( weights ) != length( badRow ) ) { #maybe add weight selection (automatially drop weights on badRows)
  ...
  if (length(weights) == 1){
    weights <- rep.int(weights, length( badRow[badRow == F] ))
    names(weights) <- names(badRow[badRow == F])
  }


  # extract the fit functions
  ...
  biprob <- do.call(bifit, list(x1 = X1, x2 = X2, x3 = X3,
                                ...
                                weights = weights,
                                control = control))
  # get the varnames right
  names(biprob$estimate) <-
    names(biprob$gradient) <-
    names(biprob$hessian) <-
    c(colnames(X1), colnames(X2), attr(Y1, "varname"), "rho120", "rho121")

  # extract the vcov (unchanged/corrected first stage)
  # clustered (twostage) standard errors
  if(!is.null(cluster)){
    cluster = cbind(data[, c(cluster)])
    kc <- ncol(cluster)
    cluster <- matrix(cluster[!badRow, ], ncol = kc)
    rownames(cluster) <- names(badRow[badRow == F])
    V1 <- ssdeR.cluster.fit(fm = biprob, dfcw = 1, cluster = cluster)
  }else{
    V1 <- vcov(biprob)
  }

  # extract the coefficients - first stage
  theta_1 = biprob$estimate

  # apply the large MM estimation (kim 2006)
  rval <- do.call(gmmfit, list(x1 = X1, x2 = X2, x3 = X3,
                              y1 = Y1, y2 = Y2, y3 = Y3,
                              V1 = V1,
                              theta = theta_1,
                              weights = weights,
  ...
  if(!is.null(cluster)){
    V2 <- ssdeR.cluster.fit(fm = rval, dfcw = 1, cluster = cluster)
  }else{
    V2 <- vcov(rval)
  }

  rval$vcov <- V2
  # is there a better way to include the first stage estimates?!
  # check that
