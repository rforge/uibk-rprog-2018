...
<<>>=
library("gevreg")
data("gevregdata", package = "gevreg")
hs <- gevregdata$hs
gev_params <- gevregdata$gev_params
grid <- gevregdata$grid
@
%
...
%
<<>>=
df <- cbind(gev_params,data.frame(lon = unique(hs$lon), alt = unique(hs$alt)))
plot(df)
@
%
...
%
<<>>=
hs$loc <- as.numeric(rep(gev_params$loc,each=nrow(hs)/length(levels(hs$station))))
...
%
<<>>=
grid$loc <- predict(m0, type = "location", newdata = grid, at=1-1/50)
@
%
...
colors <- diverge_hcl(length(values), h = c(260, 0), c = 100, l = c(30, 90), power = 0.7)
rl <- predict(m0, type = "quantile", newdata = grid, at=1-1/50)
rl.fact <- cut(rl,values,values[-1])
ggplot(data.frame(grid,rl.fact),aes(lon,lat,color=rl.fact,fill=rl.fact)) +
